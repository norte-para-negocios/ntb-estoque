...
@section('content')
    <div class="container">
        ...
        <h2 class="mb-3">{{ __('Notas fiscais - Itens') }}: <small>{{ auth()->user()->loja->nome_fantasia }}</small>
        </h2>

        <div class="card card-body">
            <h5>{{ __($recebimento->cabec->cNome ?? '') }}</h5>
            <h6>N° NFe.: {{ $recebimento->cabec->cNumeroNFe ?? '' }} |
                Emissão: {{ $recebimento->cabec->dEmissaoNFe ?? '' }}
            </h6>
                                                ...
                                                @if ($it->itensCabec->nIdProduto > 0)
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="number" class="form-control" id="quantidade"
                                                               name="quantidade"
                                                               value="{{ \App\Models\NotaFiscal::where('n_id_receb', $recebimento->cabec->nIdReceb)->where('produto_codigo', $it->itensCabec->nIdProduto)->first()->quantidade ?? 1 }}"
