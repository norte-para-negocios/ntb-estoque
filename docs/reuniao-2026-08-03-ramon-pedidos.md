...
| 11 | Relatório de Movimentações — falta detalhe por produto | Ao filtrar "movimento manual de estoque" + saída, período de 1 ano inteiro, o relatório mostra só ~R$5 mil — Ramon acha que deveria ser bem mais, e o relatório **não mostra quais produtos** compõem esse valor (só agrega). Quer poder ver por local de estoque, por produto, por família, por tipo, tanto entrada quanto saída. | **Corrigido e em produção** (commit `8cbd21f`) — drill-down família/tipo/local → produto (`lib/drill.ts`). Valor real (loja 3, 01/08/25–05/08/26): **R$136.647,36**, não ~R$5mil — não era bug de dado, era o card antigo "Perdas reais" escondendo os ajustes de inventário (R$239k na base antiga) numa métrica separada. Ver `task-6-report.md` |
