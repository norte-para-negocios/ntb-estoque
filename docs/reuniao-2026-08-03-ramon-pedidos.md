...
| 3 | Impressão de programação de produção — bug | Ao filtrar "imprimir atrasadas" por local de estoque numa loja específica, o relatório não traz dados mesmo havendo OPs atrasadas ali. Ramon disse que vai investigar ele mesmo. | **Corrigido** (Task 5 da auditoria de filtros/relatórios, commit `4c04431`) — não era o filtro de local (esse sempre funcionou); causa raiz real era o filtro de MÊS, que continuava restringindo a busca ao mês selecionado mesmo no modo "só atrasadas", escondendo OP atrasada planejada num mês anterior (o caso mais comum). Reproduzido ao vivo antes do fix, corrigido, revalidado ao vivo depois. Ver `.superpowers/sdd/2026-08-04-auditoria-filtros-relatorios/task-5-report.md` |
...
| 8 | Relatório de Faturamento — separar processado de cancelado | Ramon diz que o relatório "não traz ainda o que foi processado x cancelado" e que "nem tem filtro aqui". **Conferir se isso já não foi resolvido** nesta sessão (já existe filtro de status/cancelado em "Ver cupons") — pode ser um ponto de vista diferente do que ele quer, ou pode já estar resolvido e ele não percebeu. | **Verificado, não é bug** (Task 3 da auditoria de filtros/relatórios, commit `182013b`) — o filtro de situação (Normal/Cancelado/Devolvido/Todos) já existia e funcionava corretamente desde 31/07 (commit `c229715`); confirmado com dado real (4 cupons cancelados da loja 4 batendo 1:1 com SQL). O problema era descoberta: o filtro só existe dentro de "Ver cupons" (2 cliques), e Ramon não achou de novo. Adicionado texto fixo na visão padrão apontando onde o filtro mora. Ver `.superpowers/sdd/2026-08-04-auditoria-filtros-relatorios/task-3-report.md` |
| 9 | Relatório de Compras — não traz todas as compras do ano | Ramon afirma que o relatório de Compras não está trazendo todas as compras do ano inteiro, em todas as lojas. Contradiz trabalho recente nesta sessão (chips de status, filtros). **Precisa investigar com dado real** antes de assumir que é bug — pode ser mal-entendido de filtro padrão. | **Corrigido** (Task 2 da auditoria de filtros/relatórios, commit `4bd9867`) — bug real confirmado: a migration `097_filtro_status_compras_auditoria.sql` nunca tinha sido aplicada em produção, então as RPCs de Compras/Auditoria Fiscal chamadas com `p_status` falhavam com `function ... does not exist` em silêncio, escondendo os ~90 dias mais recentes (os "meses atuais" da queixa) em TODAS as 6 lojas. Corrigido aplicando a migration faltante + aviso visível caso a mesma classe de erro se repita. Ver `.superpowers/sdd/2026-08-04-auditoria-filtros-relatorios/task-2-report.md` |
...
| 11 | Relatório de Movimentações — falta detalhe por produto | Ao filtrar "movimento manual de estoque" + saída, período de 1 ano inteiro, o relatório mostra só ~R$5 mil — Ramon acha que deveria ser bem mais, e o relatório **não mostra quais produtos** compõem esse valor (só agrega). Quer poder ver por local de estoque, por produto, por família, por tipo, tanto entrada quanto saída. | **Corrigido e em produção** (commits `8cbd21f`/`b888a82`/`c27838a`) — drill-down família/tipo/local → produto (`lib/drill.ts`). 2 rodadas de revisão independente acharam bugs sérios no cálculo do valor (histórico completo em `task-6-report.md`, seções "Fix round 1" e "Fix round 2" — não repetir números específicos aqui, mudaram mais de uma vez). Resumo do estado atual: movimento real (ENT/SAI) fica separado de saldo contado em inventário (SLD, que é uma FOTO da contagem física, não um movimento — não soma em R$, aparece como contagem de eventos). "Delta real" do inventário (quanto de fato entrou/saiu por causa de uma contagem) não existe hoje no dado — fica como pendência, não é bug pra corrigir agora |
