-- 097_filtro_status_compras_auditoria.sql
-- Pedido real do Ramon (reunião 27/07, item #6/#28): poder escolher ver
-- Concluída/Pendente/Cancelada/Todas em Compras e Auditoria Fiscal, com
-- CONCLUIDA fixo como padrão -- igual ao filtro que já existe em Nota
-- Fiscal (lib/nf-status.ts). Até aqui essas 6 funções tinham o filtro
-- fixo no SQL (c_etapa='60' e não cancelada, migration 083/076/078/081/088),
-- sem nenhum jeito de trocar. p_status é parâmetro novo com default
-- 'CONCLUIDA' -- comportamento hoje continua idêntico pra quem não manda
-- esse parâmetro (todas as chamadas existentes usam argumento nomeado).
-- Espelhar em lib/relatorio-frio-nf.ts (feito no mesmo commit).
