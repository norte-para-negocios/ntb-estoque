-- Auditoria fiscal das compras (Módulo B). O full_object do recebimentonfe já
-- traz TODO o fiscal por item: itensAjustes.cCFOPEntrada (CFOP de entrada que a
-- empresa lançou), itensSitTribEnt.cNaoCredICMSE (não creditar ICMS),
-- cNaoGerarMovEstoque (não estoca), itensICMS.cSitTrib/cOrigem.
--
-- Visão principal: matriz por par "CFOP documento → CFOP entrada" (espelha as
-- abas de NF por CFOP do Ramon), com nº de itens, valor e quantos creditam ICMS
-- / movem estoque. Revela, p.ex., merc. comprada (5.102) lançada como uso/consumo
-- (1.556): não credita nem estoca.
...
-- Detalhe por item de um par CFOP (drill-down), paginável e ordenado.
