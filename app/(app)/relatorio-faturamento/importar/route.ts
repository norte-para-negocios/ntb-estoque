...
import { parseFaturamentoResumo, type AbaLida } from '@/lib/faturamento-import'

export const dynamic = 'force-dynamic'
const MAX = 25 * 1024 * 1024 // 25MB: o export COM a aba BD tem 306MB; tem que vir sem ela.
...
export async function POST(request: Request) {
  ...
  const linhas = parseFaturamentoResumo(abas, ano)
  if (!linhas.length) {
    return NextResponse.json(
      { error: 'Não achei as abas-resumo (Fat por família / por tipo / por forma de pgto). É o arquivo certo?' },
      { status: 400 },
    )
  }

  const supabase = createServiceClient()
  // Substitui o faturamento da loja pelo do arquivo (idempotente).
  await supabase.from('faturamento_importado').delete().eq('loja_id', lojaId)
  ...
  await registrarAuditoria('editar', 'faturamento_importado', lojaId, `Import faturamento (${linhas.length} linhas)`)
